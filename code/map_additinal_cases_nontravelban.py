...
if __name__ == "__main__":
  ...
  dfprot = pd.read_csv('./data/LocDownProt.csv')
  dfprot['city_code'] = ['CN' + str(i) for i in dfprot['city_code']]
  dfprot['city'] = [CityShift_A[i] if  i in dfprot['city'] else i for i in dfprot['city']]
  dfprot['city'] = [CityShift_B[i] if  i in dfprot['city'] else i for i in dfprot['city']]
  ...
  dfprot['Weight'] = dfprot['protect_num']
  ...
  colors['台湾'] = cmap(np.sqrt((18 - vmin) / (vmax - vmin)))[:3]
  ...
  for nshape, seg in enumerate(map.prov):
    ...
    poly = Polygon(seg, facecolor=cmap(0.001), edgecolor=color)
    ax.add_patch(poly)
  ...
  fig.tight_layout()
  plt.savefig(figures_dir / ('Additinal_cases_map_NonTravelBan.png'))

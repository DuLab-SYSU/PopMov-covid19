...
if __name__ == "__main__":
  ...
  dfrisk = pd.read_csv('./data/Final_risk_current_S1.csv')
  dfrisk['city_code'] = ['CN' + str(i) for i in dfrisk['city_code']]
  ...
  IDPro = {'CN' + str(i): j for i,
           j in zip(dfId['city_code'], dfId['province'])}
  Mycity = list(dfId['city'])

  dfrisk['city'] = [CityShift_A[i]
                    if i in CityShift_A else i for i in dfrisk['city']]
  dfrisk['city'] = [CityShift_B[i]
                    if i in CityShift_B else i for i in dfrisk['city']]
  IDrisk = {i: j for i, j in zip(dfrisk['city_code'], dfrisk['infectious'])}
  Cityrisk = {i: j for i, j in zip(dfrisk['city'], dfrisk['infectious'])}
  print(dfrisk)
  Tarcity = [i for i, j in zip(dfrisk['city'], dfrisk['infectious']) if j > 0]
  ...
  for city in Mapcity:
    if city in Cityrisk:
      if city in Tarcity:
        colors[city] = cmap(
            np.sqrt((Cityrisk[city] - vmin) / (vmax - vmin)))[:3]
      else:
        colors[city] = cmap2(
            np.sqrt((np.abs(Cityrisk[city]) - vmin) / (vmax - vmin)))[:3]
    else:
      colors[city] = cmap(0)

  colors['台湾'] = cmap(np.sqrt((Cityrisk['花莲县'] - vmin) / (vmax - vmin)))[:3]

  patches = []
  for nshape, seg in enumerate(map.prov):
    if statenames[nshape] in Tarcity:
      color = rgb2hex(colors[statenames[nshape]])
    else:
      color = 'lightgrey'
    poly = Polygon(seg, facecolor=cmap(0.01), edgecolor=cmap(0.01))
    ax.add_patch(poly)
  ...
  for nshape, seg in enumerate(map.taiwan):
    if statenames[nshape] in Tarcity:
      ...
      color = 'lightgrey'
    poly = Polygon(seg, facecolor=rgb2hex(
        colors['台湾']), edgecolor=rgb2hex(colors['台湾']))
    ax.add_patch(poly)
  ...
  cb2 = plt.colorbar(p, shrink=0.25, fraction=0.06,
                     pad=0.04, drawedges=False, aspect=5)
  font = {'family': 'arial', 'size': 8,
          }
  cb2.outline.set_linewidth(0)
  cb2.set_ticks(np.linspace(10, 20, 2))
  ...
  ax1.set_yticks([])
  Top10 = {}
  Top10['CN310000'] = IDrisk['CN310000']
  # Top10['CN440100'] = IDrisk['CN440100']
  Top10['CN110000'] = IDrisk['CN110000']
  Top10['CN430100'] = IDrisk['CN430100']
  Top10['CN320500'] = IDrisk['CN320500']
  Top10['CN510100'] = IDrisk['CN510100']
  Top10['CN340100'] = IDrisk['CN340100']
  Top10['CN411300'] = IDrisk['CN411300']
  Top10['CN500000'] = IDrisk['CN500000']
  Top10['CN430600'] = IDrisk['CN430600']
  ax.set_xlabel('Longitude', fontdict={'size': 11, 'family': 'arial'})
  ax.set_ylabel('Latitude', fontdict={'size': 11, 'family': 'arial'})
  # plt.savefig(figures_dir / ('map_work_resumption_current_scenario.png'))
  plt.show()
